...
const currencyDbOperations = require('../cruds/currency');
currencyRouter.post('/', async (req, res, next) => {
    try {
        ...
        let results = await currencyDbOperations.postCurrency(company_id, branch_id, code, description, rate_vs_usd, date, comitted_by);
        res.json(results);
    } catch (e) {
        console.log(e);
        res.sendStatus(500);
...
currencyRouter.get('/', async (req, res, next) => {
    try {
        let results = await currencyDbOperations.getCurrency();
        res.json(results);
    } catch (e) {
        console.log(e);
        res.sendStatus(500);
...
currencyRouter.get('/:id', async (req, res, next) => {
    try {
        ...
        let result = await currencyDbOperations.getCurrencyById(id);
        res.json(result);
    } catch (e) {
        console.log(e);
        res.sendStatus(500);
...
currencyRouter.put('/:id', async (req, res, next) => {
    try {
      ...
      let result = await currencyDbOperations.updateCurrency(
        id,
        company_id,
        branch_id,
        code,
...
currencyRouter.delete('/:id', async (req, res, next) => {
    try {
        let id = req.params.id;
        let result = await currencyDbOperations.deleteCurrency(id);
        res.json(result);
    } catch (e) {
        console.log(e);
        res.sendStatus(500);
